...
if len(all_features) == 0:
    print("❌ No data collected! Check your internet connection and Binance API.")
    exit(1)
...
# Use the updated conversion method
try:
    # For coremltools 6.2
    coreml_model = ct.converters.sklearn.convert(model, feature_names, 'target')
except Exception as e:
    print(f"⚠️ First conversion method failed: {e}")
    try:
        # Alternative method
        coreml_model = ct.convert(
            model,
            source="scikit-learn",
            inputs=[ct.ImageType(name=name) for name in feature_names]  # This is a simplification - you may need to adjust
        )
    except Exception as e:
        print(f"❌ Conversion failed: {e}")
        print("Saving model using pickle instead...")
        import pickle
        with open('TradingModel.pkl', 'wb') as f:
            pickle.dump(model, f)
        print("✅ Model saved as TradingModel.pkl")
        exit(0)
