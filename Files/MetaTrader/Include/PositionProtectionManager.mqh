...
private:
   ...
   void ManagePosition(const ulong ticket){if(!PositionSelectByTicket(ticket)||((ulong)PositionGetInteger(POSITION_MAGIC)!=m_magic))return;const string symbol=PositionGetString(POSITION_SYMBOL);const ENUM_POSITION_TYPE type=(ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);const double open=PositionGetDouble(POSITION_PRICE_OPEN),current=PositionGetDouble(POSITION_VOLUME),original=OriginalVolume(ticket,current);if(original<=0.0||current<=0.0)return;const double profit=ProfitPips(symbol,type,open),tp1=MathMax(0.0,Setting(FS_TP1_PIPS,10.0)),tp2=MathMax(tp1,Setting(FS_TP2_PIPS,15.0)),tp3=MathMax(tp2,Setting(FS_TP3_PIPS,20.0)),p1=MathMax(0.0,MathMin(1.0,Setting(FS_TP1_PCT,0.50))),p2=MathMax(0.0,MathMin(1.0,Setting(FS_TP2_PCT,0.30))),p3=MathMax(0.0,MathMin(1.0,Setting(FS_TP3_PCT,0.20)));if(profit>=tp1&&!Done(ticket,"TP1")){if(ClosePartial(ticket,symbol,original*p1))MarkDone(ticket,"TP1");}if(!PositionSelectByTicket(ticket))return;if(profit>=tp1&&!Done(ticket,"BE")){if(ModifyStopForwardOnly(ticket,symbol,type,open))MarkDone(ticket,"BE");}if(!PositionSelectByTicket(ticket))return;if(profit>=tp2&&!Done(ticket,"TP2")){if(ClosePartial(ticket,symbol,original*p2))MarkDone(ticket,"TP2");}if(!PositionSelectByTicket(ticket))return;if(profit>=tp3&&!Done(ticket,"TP3")){if(ClosePartial(ticket,symbol,MathMin(original*p3,PositionGetDouble(POSITION_VOLUME))))MarkDone(ticket,"TP3");}if(!PositionSelectByTicket(ticket))return;const double activation=MathMax(1.0,Setting(FS_TRAIL_ACTIVATION,5.0));if(profit<activation)return;const double pip=PipSize(symbol);if(pip<=0.0)return;MqlTick tick;if(!SymbolInfoTick(symbol,tick))return;const double dist=TrailDistancePips(profit),ref=type==POSITION_TYPE_BUY?tick.bid:tick.ask,target=type==POSITION_TYPE_BUY?ref-dist*pip:ref+dist*pip;ModifyStopForwardOnly(ticket,symbol,type,target);}
public:
   void InitializePosition(const ulong ticket){if(!PositionSelectByTicket(ticket)||((ulong)PositionGetInteger(POSITION_MAGIC)!=m_magic))return;const string symbol=PositionGetString(POSITION_SYMBOL);const double volume=PositionGetDouble(POSITION_VOLUME);OriginalVolume(ticket,volume);const double sl=PositionGetDouble(POSITION_SL),tp=PositionGetDouble(POSITION_TP);if(tp>0.0){if(m_trade.PositionModify(ticket,sl,0.0))PrintFormat("[EA V22] PROTECTION INIT | ticket=%I64u | fixed TP %.5f removed; staged TP manager active",ticket,tp);else PrintFormat("[EA V22] WARNING | ticket=%I64u | could not remove fixed TP %.5f | retcode=%u | comment=%s",ticket,tp,m_trade.ResultRetcode(),m_trade.ResultRetcodeDescription());}}
