...
#define PM23_MAGIC_NUMBER 888888
#define PM23_PREFIX "FSV23_PM_"
#define PM23_STAGE_TP1 1
#define PM23_STAGE_TP2 2
#define PM23_STAGE_TP3 3
...
private:
   ...
   {
      return PM23_PREFIX + IntegerToString((long)ticket) + "_" + suffix;
   }
   ...
   double OriginalVolume(ulong ticket, double current, double tp1Pct, double tp2Pct, double tp3Pct)
   {
      ...
      double reconstructed = MathMax(current, current + ClosedVolumeFromHistory(ticket));
      ...
      double step = SymbolInfoDouble(symbol, SYMBOL_VOLUME_STEP);
      double tolerance = reconstructed > 0.0 ? (0.5 * MathMax(step, 0.00000001) / reconstructed) : 0.0;
      int inferredStage = 0;
      if(tp1Pct > 0.0 && fraction + tolerance >= tp1Pct) inferredStage = PM23_STAGE_TP1;
      if(tp2Pct > 0.0 && fraction + tolerance >= tp1Pct + tp2Pct) inferredStage = PM23_STAGE_TP2;
      if(tp3Pct > 0.0 && fraction + tolerance >= tp1Pct + tp2Pct + tp3Pct - 0.000001) inferredStage = PM23_STAGE_TP3;
   ...
   bool VerifySL(ulong ticket, double expected, int digits, double tolerance)
   {
      ...
      {
         if(PositionSelectByTicket(ticket) && MathAbs(PositionGetDouble(POSITION_SL) - expected) <= tolerance) return true;
         Sleep(100);
      }
      return false;
   }
   ...
   bool ModifySL(ulong ticket, string symbol, ENUM_POSITION_TYPE type, double newSL, string reason)
   {
      ...
      {
         PrintFormat("[EA V23 PM] SL SKIP | ticket=%I64d | reason=%s | requestedSL=%s | broker stop/freeze distance not satisfied", ticket, reason, DoubleToString(newSL, digits));
         return false;
      }

      m_trade.SetExpertMagicNumber(PM23_MAGIC_NUMBER);
      m_trade.SetTypeFillingBySymbol(symbol);
      m_trade.SetAsyncMode(false);
      bool callOK = m_trade.PositionModify(ticket, newSL, currentTP);
      uint retcode = m_trade.ResultRetcode();
      bool accepted = callOK && (SuccessfulRetcode(retcode) || retcode == TRADE_RETCODE_PLACED);
      bool verified = accepted && VerifySL(ticket, newSL, digits, MathMax(point * 2.0, 0.00000001));
      double resultingTP = PositionSelectByTicket(ticket) ? PositionGetDouble(POSITION_TP) : 0.0;
      double resultingSL = PositionSelectByTicket(ticket) ? PositionGetDouble(POSITION_SL) : 0.0;
      PrintFormat("[EA V23 PM] SL MODIFY | ticket=%I64d | reason=%s | requestedSL=%s | requestedTP=%s | resultingSL=%s | resultingTP=%s | retcode=%u | accepted=%s | verified=%s | %s",
                  ticket, reason, DoubleToString(newSL, digits), DoubleToString(currentTP, digits),
                  DoubleToString(resultingSL, digits), DoubleToString(resultingTP, digits), retcode,
                  accepted ? "true" : "false", verified ? "true" : "false", m_trade.ResultRetcodeDescription());
      return verified;
   }
   ...
   bool VerifyClose(ulong ticket, double beforeVolume, double requestedVolume, string symbol, string reason, double &executedOut)
   {
      ...
      for(int attempt = 0; attempt < 8; attempt++)
      {
         ...
         {
            executedOut = beforeVolume;
            PrintFormat("[EA V23 PM] CLOSE VERIFY | ticket=%I64d | reason=%s | requested=%.8f | executed=%.8f | remaining=0 | verified=true", ticket, reason, requestedVolume, beforeVolume);
            return true;
         }
         ...
         {
            executedOut = executed;
            PrintFormat("[EA V23 PM] CLOSE VERIFY | ticket=%I64d | reason=%s | requested=%.8f | executed=%.8f | remaining=%.8f | verified=true", ticket, reason, requestedVolume, executed, afterVolume);
            return true;
         }
         Sleep(100);
      }
      ...
      PrintFormat("[EA V23 PM] CLOSE VERIFY | ticket=%I64d | reason=%s | requested=%.8f | executed=%.8f | remaining=%.8f | verified=false", ticket, reason, requestedVolume, MathMax(0.0, beforeVolume - remaining), remaining);
      return false;
   }

   bool CloseVolume(ulong ticket, string symbol, ENUM_POSITION_TYPE type, double requested, string reason)
   {
      if(!PositionSelectByTicket(ticket)) return false;
      ...
      double closeVolume = NormalizeCloseVolume(symbol, requested, beforeVolume);
      ...
      if(closeVolume >= beforeVolume - step * 0.25)
         ...
         callOK = m_trade.Sell(closeVolume, symbol, 0.0, 0.0, 0.0, "GOD_MODE_V23_TP");
      else
         callOK = m_trade.Buy(closeVolume, symbol, 0.0, 0.0, 0.0, "GOD_MODE_V23_TP");

      uint retcode = m_trade.ResultRetcode();
      bool accepted = callOK && (SuccessfulRetcode(retcode) || retcode == TRADE_RETCODE_PLACED);
      double executed = 0.0;
      bool verified = accepted && VerifyClose(ticket, beforeVolume, closeVolume, symbol, reason, executed);
      PrintFormat("[EA V23 PM] CLOSE | ticket=%I64d | reason=%s | requested=%.8f | brokerReported=%.8f | retcode=%u | accepted=%s | verified=%s | resultingPosition=%s | %s",
                  ticket, reason, closeVolume, m_trade.ResultVolume(), retcode,
                  accepted ? "true" : "false", verified ? "true" : "false",
   ...
   void ManagePosition(ulong ticket)
   {
      ...
      if((long)PositionGetInteger(POSITION_MAGIC) != PM23_MAGIC_NUMBER) return;
      ...
      if(profitPips > 0.0 && stage < PM23_STAGE_TP1 && tp1Pct > 0.0 && tp1Pips > 0.0 && profitPips >= tp1Pips)
      {
         if(CloseVolume(ticket, symbol, type, original * tp1Pct, "TP1"))
         {
            SetTPStage(ticket, PM23_STAGE_TP1);
            stage = PM23_STAGE_TP1;
            PrintFormat("[EA V23 PM] TP1 COMPLETE | ticket=%I64d | original=%.8f | configuredClose=%.2f%% | trigger=%.2f pips", ticket, original, tp1Pct * 100.0, tp1Pips);
         }
      }
      ...
      if(profitPips > 0.0 && stage < PM23_STAGE_TP2 && tp2Pct > 0.0 && tp2Pips > 0.0 && profitPips >= tp2Pips)
      {
         if(CloseVolume(ticket, symbol, type, original * tp2Pct, "TP2"))
         {
            SetTPStage(ticket, PM23_STAGE_TP2);
            stage = PM23_STAGE_TP2;
            PrintFormat("[EA V23 PM] TP2 COMPLETE | ticket=%I64d | original=%.8f | configuredClose=%.2f%% | trigger=%.2f pips", ticket, original, tp2Pct * 100.0, tp2Pips);
         }
      }
      ...
      profitPips = type == POSITION_TYPE_BUY ? (current - entry) / pip : (entry - current) / pip;
      stage = TPStage(ticket);
      if(profitPips > 0.0 && stage < PM23_STAGE_TP3 && tp3Pips > 0.0 && profitPips >= tp3Pips)
      {
         double remaining = PositionGetDouble(POSITION_VOLUME);
         if(remaining > 0.0 && CloseVolume(ticket, symbol, type, remaining, "TP3_FINAL"))
         {
            SetTPStage(ticket, PM23_STAGE_TP3);
            PrintFormat("[EA V23 PM] TP3 COMPLETE | ticket=%I64d | finalRemaining=%.8f | configuredTarget=%.2f%% | trigger=%.2f pips", ticket, remaining, tp3Pct * 100.0, tp3Pips);
            return;
         }
      }
      ...
      if(breakevenEnabled && beTrigger > 0.0 && profitPips >= beTrigger && !BreakevenDone(ticket))
      {
         ...
         if(ModifySL(ticket, symbol, type, beSL, "BREAKEVEN"))
         {
            ...
            PrintFormat("[EA V23 PM] BREAKEVEN COMPLETE | ticket=%I64d | entry=%s | offset=%.2f pips", ticket, DoubleToString(entry, (int)SymbolInfoInteger(symbol, SYMBOL_DIGITS)), beOffset);
         }
      }
      ...
      double trailStep = MathMax(0.0, ReadSetting("TRAIL_STEP_PIPS", 1.0));
      if(trailingEnabled && activation > 0.0 && distance > 0.0 && profitPips >= activation)
      {
         ...
         double movement = existingSL > 0.0 ? MathAbs(desired - existingSL) / pip : trailStep;
         if(ImproveOnly(type, existingSL, desired) && (existingSL <= 0.0 || movement >= trailStep))
   ...
   {
      if(GlobalVariableCheck(PM23_PREFIX + "CONFIG_READY") && GlobalVariableGet(PM23_PREFIX + "CONFIG_READY") > 0.5) return;
      GlobalVariableSet(PM23_PREFIX + "TP1_PCT", 0.50);
      GlobalVariableSet(PM23_PREFIX + "TP1_PIPS", 10.0);
      GlobalVariableSet(PM23_PREFIX + "TP2_PCT", 0.30);
      GlobalVariableSet(PM23_PREFIX + "TP2_PIPS", 15.0);
      GlobalVariableSet(PM23_PREFIX + "TP3_PCT", 0.20);
      GlobalVariableSet(PM23_PREFIX + "TP3_PIPS", 20.0);
      GlobalVariableSet(PM23_PREFIX + "BE_ENABLED", 1.0);
      GlobalVariableSet(PM23_PREFIX + "BE_TRIGGER_PIPS", 10.0);
      GlobalVariableSet(PM23_PREFIX + "BE_OFFSET_PIPS", 0.0);
      GlobalVariableSet(PM23_PREFIX + "TRAIL_ENABLED", 1.0);
      GlobalVariableSet(PM23_PREFIX + "TRAIL_ACTIVATION_PIPS", 5.0);
      GlobalVariableSet(PM23_PREFIX + "TRAIL_DISTANCE_PIPS", 6.0);
      GlobalVariableSet(PM23_PREFIX + "TRAIL_STEP_PIPS", 1.0);
      GlobalVariableSet(PM23_PREFIX + "CONFIG_READY", 1.0);
      GlobalVariablesFlush();
      Print("[EA V23 PM] DEFAULT CONFIGURATION INITIALIZED | Swift settings sync may overwrite these values");
   }
