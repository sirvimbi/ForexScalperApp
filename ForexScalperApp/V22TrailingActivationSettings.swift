...
/// V22 trailing activation is deliberately independent from hard/structural SL.
/// The EA remains authoritative for broker-side protection and the trailing curve.
@MainActor
final class V22TrailingActivationController: ObservableObject {
    ...
    init() {
        ...
        activationPips = saved > 0 ? min(100, max(1, saved)) : 5.0
    }

    func save() {
        activationPips = min(100, max(1, activationPips))
    ...
    var body: some View {
        GlassCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("Controls only when trailing activates. Hard/structural/ATR SL remains unchanged. The EA only moves SL forward and broker validation remains active.")
                    .font(.system(size: 10))
...
actor MT5V22TrailingSettingsService {
    ...
    private var baseURL: String {
        ...
        return raw.replacingOccurrences(of: ":8891", with: ":8890")
    }
