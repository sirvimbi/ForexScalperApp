...
final class V22TrailingActivationController: ObservableObject {
    ...
    init() { let saved = UserDefaults.standard.double(forKey: defaultsKey); activationPips = saved > 0 ? min(50, max(1, saved)) : 5.0 }
    func save() {
        activationPips = min(50, max(1, activationPips)); UserDefaults.standard.set(activationPips, forKey: defaultsKey)
        Task { @MainActor in
            do { let value = try await MT5V22TrailingSettingsService.shared.setActivationPips(activationPips); syncStatus = String(format: "MT5 V22 synced • %.1f pips", value); godLog(String(format: "🛡️ V22 TRAILING | activation saved=%.1f pips | hard SL/curve unchanged", value), level: .success) }
            catch { syncStatus = "Saved locally • MT5 sync pending"; godLog("⚠️ V22 TRAILING | local setting saved but MT5 sync failed: \(error.localizedDescription)", level: .warning) }
        }
    }
    func refreshFromMT5() {
        Task { @MainActor in
            do { let value = try await MT5V22TrailingSettingsService.shared.getActivationPips(); activationPips = value; UserDefaults.standard.set(value, forKey: defaultsKey); syncStatus = String(format: "MT5 V22 synced • %.1f pips", value) }
            catch { syncStatus = "MT5 unavailable" }
        }
    }
}
    ...
    var body: some View {
        GlassCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                HStack(spacing: 12) { Text("Activation").font(.system(size: 12)).foregroundColor(.textSecondary); Slider(value: $controller.activationPips, in: 1...50, step: 0.5); Text(String(format: "%.1f pips", controller.activationPips)).font(.system(size: 12, weight: .bold, design: .monospaced)).foregroundColor(.accentGold).frame(width: 75, alignment: .trailing) }
                HStack(spacing: 8) { Text("Fixed curve").font(.system(size: 10, weight: .bold, design: .monospaced)).foregroundColor(.accentCyan); Text("5–10: 3 • 10–15: 4.5 • 15–25: 6 • 25–40: 8 • 40–80: 10 • 80+: 12 pips").font(.system(size: 9, design: .monospaced)).foregroundColor(.textMuted).lineLimit(2) }
                HStack(spacing: 10) {
                ...
                HStack(spacing: 8) {
                    ...
                    Button("RELOAD") { accuracyController.reload() }.font(.system(size: 10, weight: .bold, design: .monospaced)).foregroundColor(.accentPurple).padding(.horizontal, 12).padding(.vertical, 7).background(Color.accentPurple.opacity(0.10)).cornerRadius(6).buttonStyle(.plain)
                }
                Divider().background(Color.borderSubtle)
...
actor MT5V22TrailingSettingsService {
    ...
    func setActivationPips(_ pips: Double) async throws -> Double { try await request(method: "POST", pips: min(100, max(1, pips))) }
