// ScalpingSignalEngine.swift - GOD MODE V7.2 ELITE (Config-Aware Candle Validation)
import Foundation
...
actor ScalpingSignalEngine {
    ...
    func evaluateScalpingSignal(symbol: String) async -> ScalpingSignal? {
        ...
        let (enabledNews, highMin, medMin, cooldown, spreadTol, rocPeriod, htfWeight) = await MainActor.run {
            ...
            return (config.enableNewsFilter, config.pauseBeforeHighImpactMinutes,
                    ...
                    config.spreadTolerance, config.rocPeriod, config.weightHTFAlignment)
        }
        ...
        guard await validateData(candlesByTimeframe, symbol: symbol, htfRequired: htfWeight > 0) else { return nil }
    ...
    private func getCachedTrend(symbol: String, timeframe: String) async -> SignalType {
        if let cached = cachedHTFTrends[symbol]?[timeframe], Date().timeIntervalSince(cached.timestamp) < htfCacheDuration { return cached.trend }
    ...
    private func calculateFastIndicators(symbol: String, candles1m: [Kline], h4Trend: SignalType, d1Trend: SignalType) async -> IndicatorSet {
        ...
        return IndicatorSet(rsi: rsi, stochasticK: stoch.k.last ?? 50, stochasticD: stoch.d.last ?? 50,
                            ...
                            ema9: ema9, ema21: ema21, ema50: ema50, ema9_5m: ema9, ema21_5m: ema21, ema50_5m: ema50,
                            bbPosition: bbPos, volumeRatio: 1.0, volumeProfilePOC: 0, support: 0, resistance: 0,
                            sessions: (asiaRange: (0,0), londonRange: (0,0), usRange: (0,0)), trendStrength: 0,
                            pricePattern: .none, regime: .ranging, currentPrice: currentPrice, h4Trend: h4Trend,
                            d1Trend: d1Trend, w1Trend: .none, momentumScore: momentum, isAccelerating: isAccel,
                            fvgGaps: gaps, deltaVolume: delta)
    }
    ...
    func updateSignalQuality(symbol: String, type: SignalType, confidence: Double, wasWin: Bool) async {
        ...
        if let index = history.lastIndex(where: { $0.type == type && abs($0.confidence - confidence) < 5 && $0.wasWin == nil }) { history[index].wasWin = wasWin }
    ...
    private func calculateAllIndicators(symbol: String, candlesByTimeframe: [String: [Kline]], rocPeriod: Int) async -> IndicatorSet {
        ...
        return IndicatorSet(rsi: rsi, stochasticK: stoch.k.last ?? 50, stochasticD: stoch.d.last ?? 50,
                            ...
                            ema9: ema9, ema21: ema21, ema50: ema50, ema9_5m: ema9_5m, ema21_5m: ema21_5m, ema50_5m: ema50_5m,
                            bbPosition: bbPosition, volumeRatio: 1.0, volumeProfilePOC: 0, support: 0, resistance: 0,
                            sessions: sessions, trendStrength: 0, pricePattern: .none, regime: .ranging,
    ...
    private func generateSignal(symbol: String, indicators: IndicatorSet, candles1m: [Kline]) async -> ScalpingSignal {
        let (deltaThreshold, mlThreshold, weights, pullbackEMAPeriod) = await MainActor.run {
            let config = ScalpingConfig.shared
            return (config.orderFlowThreshold, config.mlConfidenceThreshold,
        ...
        if indicators.h4Trend == .buy && indicators.d1Trend == .buy {
            buyScore += htfWeight; factors["Institutional Alignment"] = htfWeight
            ...
            sellScore += htfWeight; factors["Institutional Alignment"] = htfWeight
        ...
        if indicators.rsi < rsiOversold && indicators.stochasticK < stochOversold {
            buyScore += momWeight; factors["Dip Buy"] = momWeight
            ...
            sellScore += momWeight; factors["Rally Sell"] = momWeight
        ...
        if mlDir == type && mlConf >= mlThreshold { confidence = min(confidence + (mlConf * mlWeight), 99); factors["ML Confirmed"] = mlConf * mlWeight }
        else if mlDir != .none && mlConf > (mlThreshold + 0.1) { confidence *= 0.7; factors["ML Divergence"] = -30; godLog("⚠️ ML DIVERGENCE | \(symbol) | ruleEngine=\(type) model=\(mlDir) | confidence reduced symmetrically", level: .warning) }
    ...
    private func validateData(_ dict: [String: [Kline]], symbol: String, htfRequired: Bool) async -> Bool {
        // Only timeframes actually required by the active strategy are hard gates.
        // M1/M5 are the LTF execution inputs. H4/D1 are required only when the
        // configured HTF alignment pillar has a non-zero weight. 15m/30m/1h/W1
        // are informational in this engine and must never block a signal.
        let required: [(String, Int)] = htfRequired
            ? [("1m", 100), ("5m", 100), ("4h", 50), ("D1", 50)]
            : [("1m", 100), ("5m", 100)]
        let requiredMap = Dictionary(uniqueKeysWithValues: required)
        var valid = true

        for tf in timeframes {
            let count = dict[tf]?.count ?? 0
            if let minimum = requiredMap[tf] {
                if count < minimum {
                    valid = false
                    godLog("🛑 CANDLE VALIDATION | \(symbol) | \(tf)=\(count), required>=\(minimum) | REQUIRED", level: .warning)
                } else {
                    godLog("✅ CANDLE VALIDATION | \(symbol) | \(tf)=\(count), required>=\(minimum) | REQUIRED", level: .info)
                }
            } else if count == 0 {
                godLog("ℹ️ CANDLE VALIDATION | \(symbol) | \(tf)=0 | OPTIONAL — does not block signal", level: .info)
            } else {
                godLog("ℹ️ CANDLE VALIDATION | \(symbol) | \(tf)=\(count) | OPTIONAL — does not block signal", level: .info)
            }
        }

        godLog("📋 CANDLE VALIDATION SUMMARY | \(symbol) | required=\(required.map { $0.0 }.joined(separator: ",")) | HTF=\(htfRequired ? "ENABLED" : "DISABLED") | result=\(valid ? "PASS" : "BLOCK")", level: valid ? .info : .warning)
        return valid
    }
