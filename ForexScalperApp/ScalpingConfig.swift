...
class ScalpingConfig: ObservableObject {
    ...
    struct SymbolConfig { let minConfidence: Double; let baseSL: Double; let baseTP: Double; let maxSpread: Double }
    struct SessionMultiplier { let confidence: Double; let sl: Double; let tp: Double; let spread: Double }
    ...
    // V22: activation threshold only. It does not alter hard/structural/ATR SL,
    // broker stop validation, the trailing curve, or the forward-only rule.
    @Published var trailingActivationPips: Double = 5.0
    ...
    init() {
        savePath = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0].appendingPathComponent("scalping_config.json")
        loadConfig()
    }

    func getConfidenceThreshold(for symbol: String) -> Double { confidenceThreshold }
    func getSymbolConfig(_ symbol: String) -> SymbolConfig { SymbolConfig(minConfidence: confidenceThreshold, baseSL: baseSLPips, baseTP: baseTPPips, maxSpread: spreadTolerance) }
    func getSessionMultiplier() -> SessionMultiplier {
        let hour = Calendar.current.component(.hour, from: Date())
        if hour < 8 { return sessionMultipliers["Asian"] ?? SessionMultiplier(confidence: 1, sl: 1, tp: 1, spread: 1) }
        if hour < 16 { return sessionMultipliers["London"] ?? SessionMultiplier(confidence: 1, sl: 1, tp: 1, spread: 1) }
        return sessionMultipliers["US"] ?? SessionMultiplier(confidence: 1, sl: 1, tp: 1, spread: 1)
    }
    ...
    private func loadConfig() {
        do {
            ...
            fixedSLPips = config.fixedSLPips ?? 30
            ...
            trailingActivationPips = min(100, max(1, config.trailingActivationPips ?? 5))
            weightHTFAlignment = config.weightHTFAlignment ?? 25
            weightMomentumExhaustion = config.weightMomentumExhaustion ?? 15
            weightVolumeSurge = config.weightVolumeSurge ?? 12
            weightEMAStack = config.weightEMAStack ?? 18
            weightBollingerRejection = config.weightBollingerRejection ?? 10
            weightCCICycle = config.weightCCICycle ?? 10
            weightSARTrend = config.weightSARTrend ?? 10
            weightMomentumSurge = config.weightMomentumSurge ?? 12
            weightOrderFlow = config.weightOrderFlow ?? 15
            weightMLConfirmed = config.weightMLConfirmed ?? 10
            partialTP1_Percent = config.partialTP1_Percent ?? 0.50
            partialTP1_Pips = config.partialTP1_Pips ?? 10
            partialTP2_Percent = config.partialTP2_Percent ?? 0.30
            partialTP2_Pips = config.partialTP2_Pips ?? 15
            partialTP3_Percent = config.partialTP3_Percent ?? 0.20
            partialTP3_Pips = config.partialTP3_Pips ?? 20
        } catch { saveConfig() }
    }
    ...
    func saveConfig() {
        let config = ConfigData(
            ...
            trailingActivationPips: trailingActivationPips, weightHTFAlignment: weightHTFAlignment, weightMomentumExhaustion: weightMomentumExhaustion,
            weightVolumeSurge: weightVolumeSurge, weightEMAStack: weightEMAStack, weightBollingerRejection: weightBollingerRejection,
            weightCCICycle: weightCCICycle, weightSARTrend: weightSARTrend, weightMomentumSurge: weightMomentumSurge,
            weightOrderFlow: weightOrderFlow, weightMLConfirmed: weightMLConfirmed, partialTP1_Percent: partialTP1_Percent,
            partialTP1_Pips: partialTP1_Pips, partialTP2_Percent: partialTP2_Percent, partialTP2_Pips: partialTP2_Pips,
            partialTP3_Percent: partialTP3_Percent, partialTP3_Pips: partialTP3_Pips)
        do {
    ...
    private struct ConfigData: Codable {
        let confidenceThreshold: Double; let spreadTolerance: Double; let minScore: Double; let cooldownSeconds: Double
        let minVolatilityATR: Double; let minVolumeRatio: Double; let mandatoryConfluenceLevel: Int; let minConfluencePillars: Double
        let maxHoldMinutes: Double; let enableIndicatorExit: Bool; let maxDailyTrades: Int; let maxConcurrentScalps: Int
        let enableHourlyLimit: Bool?; let maxHourlyTrades: Int?; let enableRRCheck: Bool?; let minRRRatio: Double?
        let fixedSLPips: Double?; let useFixedSL: Bool?; let baseSLPips: Double; let baseTPPips: Double; let maxTPPips: Double; let minTPPips: Double
        let partialTPPercent: Double; let brokerSuffix: String?; let manualLotSize: Double?; let enableNewsFilter: Bool?
        let autoRaiseSpreadDuringNews: Bool?; let newsSpreadMultiplier: Double?
        let enableOrderFlowFilter: Bool?; let orderFlowThreshold: Double?; let enablePullbackEntry: Bool?; let pullbackEMAPeriod: Int?
        let pullbackFVGWeight: Double?; let rocPeriod: Int?; let enableMLTrendFilter: Bool?; let mlConfidenceThreshold: Double?
        let enableSwingSL: Bool?; let swingLookback: Int?; let volatilityMultiplierMin: Double?; let volatilityMultiplierMax: Double?
        let trailingActivationPips: Double?
        let weightHTFAlignment: Double?; let weightMomentumExhaustion: Double?; let weightVolumeSurge: Double?; let weightEMAStack: Double?
        let weightBollingerRejection: Double?; let weightCCICycle: Double?; let weightSARTrend: Double?; let weightMomentumSurge: Double?
        let weightOrderFlow: Double?; let weightMLConfirmed: Double?
        let partialTP1_Percent: Double?; let partialTP1_Pips: Double?; let partialTP2_Percent: Double?; let partialTP2_Pips: Double?
        let partialTP3_Percent: Double?; let partialTP3_Pips: Double?
    }
}
