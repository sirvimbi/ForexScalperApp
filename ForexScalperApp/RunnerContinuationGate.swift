...
struct RunnerContinuationGate: Sendable {
    ...
    func evaluate(direction: String, candles: [RunnerCandle], keyLevel: Double? = nil, atr: Double? = nil) -> RunnerContinuationResult {
        ...
        guard candles.count >= lookback else { return result(false, "insufficient closed candles (need \(lookback))", 0, 0, 0, 0, 0, false) }
        ...
        guard aligned >= minimumAligned else { return result(false, "candle alignment \(aligned)/\(lookback)", aligned, 0, 0, latest.bodyToRange, wickRatio, false) }
        guard !configuration.requireLatestCandleAlignment || latestAligned else { return result(false, "latest closed candle disagrees with signal", aligned, 0, 0, latest.bodyToRange, wickRatio, false) }

        if configuration.requireProgressiveCloses {
            for index in 1..<recent.count {
                let previous = recent[index - 1]
                let current = recent[index]
                if isBuy && current.close <= previous.close { return result(false, "bullish closes are not progressive", aligned, 0, 0, latest.bodyToRange, wickRatio, false) }
                if isSell && current.close >= previous.close { return result(false, "bearish closes are not progressive", aligned, 0, 0, latest.bodyToRange, wickRatio, false) }
            }
        }
        ...
        guard acceleration >= configuration.minimumAccelerationRatio else { return result(false, String(format: "momentum not accelerating (%.2fx)", acceleration), aligned, acceleration, 0, latest.bodyToRange, wickRatio, false) }
        guard latest.bodyToRange >= configuration.minimumBodyToRangeRatio else { return result(false, String(format: "weak candle body (%.2f)", latest.bodyToRange), aligned, acceleration, 0, latest.bodyToRange, wickRatio, false) }
        guard wickRatio <= configuration.maximumOpposingWickToBodyRatio else { return result(false, String(format: "poor wick quality (%.2fx)", wickRatio), aligned, acceleration, 0, latest.bodyToRange, wickRatio, false) }
        ...
        if let keyLevel, let atr, atr > 0 {
            ...
            guard extensionATR <= configuration.maximumBreakoutExtensionATR else { return result(false, String(format: "breakout overextended (%.2f ATR)", extensionATR), aligned, acceleration, extensionATR, latest.bodyToRange, wickRatio, false) }
        }
        ...
        guard !antiRunner else { return result(false, "anti-runner exhaustion pattern", aligned, acceleration, extensionATR, latest.bodyToRange, wickRatio, true) }
    ...
    private func result(_ passed: Bool, _ reason: String, _ aligned: Int, _ acceleration: Double, _ extensionATR: Double, _ body: Double, _ wick: Double, _ antiRunner: Bool) -> RunnerContinuationResult {
        RunnerContinuationResult(passed: passed, reason: reason, alignedCandles: aligned, accelerationRatio: acceleration, extensionATR: extensionATR, latestBodyToRange: body, latestOpposingWickRatio: wick, antiRunnerTriggered: antiRunner)
    }
}
