...
final class DailyNewsIntelligence {
    ...
    func publishIfNeeded(force: Bool = false) async {
        ...
        let events = NewsService.shared.upcomingEvents.filter { calendar.isDate($0.time, inSameDayAs: now) && $0.time >= now.addingTimeInterval(-3600) }
        ...
        let watchCurrencies = currencyScores.sorted { abs($0.value) > abs($1.value) }.prefix(6)
        let watchPairs = makePairBiases(watchCurrencies.map { ($0.key, $0.value) })
        let top = watchPairs.prefix(6).map { "\($0.symbol)=\($0.bias) (\(String(format: "%+.2f", $0.score)))" }.joined(separator: " | ")
        let driverText = watchCurrencies.prefix(4).map { "\($0.key): \(Array(Set(drivers[$0.key] ?? [])).prefix(2).joined(separator: ", "))" }.joined(separator: " • ")
    ...
    private func eventScore(_ event: NewsEvent) -> Double {
        let impact = event.impact == .high ? 3.0 : event.impact == .medium ? 1.5 : 0.5
    ...
    private func makePairBiases(_ currencies: [(String, Double)]) -> [(symbol: String, score: Double, bias: String)] {
        ...
        return pairs.map { pair in
            ...
            return (pair, score, score > 1.0 ? "BULLISH" : score < -1.0 ? "BEARISH" : "NEUTRAL")
