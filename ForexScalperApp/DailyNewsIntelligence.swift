...
final class DailyNewsIntelligence {
    ...
    func publishIfNeeded(force: Bool = false) async {
        let settings = DailyNewsConfiguration.load()
        ...
        let events = NewsService.shared.upcomingEvents.filter {
            calendar.isDate($0.time, inSameDayAs: now) && $0.time >= now.addingTimeInterval(-settings.eventLookbackHours * 3600)
        }
        ...
        for event in events {
            currencyScores[event.currency, default: 0] += eventScore(event, settings: settings)
            drivers[event.currency, default: []].append(event.title)
        }
        let watchCurrencies = currencyScores.sorted { abs($0.value) > abs($1.value) }.prefix(settings.watchCurrencyCount)
        let watchPairs = makePairBiases(watchCurrencies.map { ($0.key, $0.value) }, settings: settings)
        ...
        let insight = GodModeInsight(id: UUID(), type: .newsBroadcast, symbol: "MACRO", title: "DAILY NEWS WATCHLIST",
    ...
    private func eventScore(_ event: NewsEvent, settings: DailyNewsConfiguration) -> Double {
        let impact = event.impact == .high ? settings.highImpactWeight : event.impact == .medium ? settings.mediumImpactWeight : settings.lowImpactWeight
        let title = event.title.lowercased()
        guard let f = numeric(event.forecast), let p = numeric(event.previous) else {
            return (title.contains("rate") || title.contains("cpi") || title.contains("inflation") || title.contains("employment") || title.contains("nfp")) ? impact * 0.15 : 0
        }
        var direction = f - p
    ...
    private func makePairBiases(_ currencies: [(String, Double)], settings: DailyNewsConfiguration) -> [(symbol: String, score: Double, bias: String)] {
        ...
        return pairs.map { pair in
            ...
            return (pair, score, score > settings.pairBiasThreshold ? "BULLISH" : score < -settings.pairBiasThreshold ? "BEARISH" : "NEUTRAL")
