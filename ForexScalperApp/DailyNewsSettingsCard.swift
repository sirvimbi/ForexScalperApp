...
struct DailyNewsSettingsCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Image(systemName: "newspaper.fill").foregroundColor(.accentGold)
                Text("DAILY NEWS INTELLIGENCE").font(.system(size: 11, weight: .bold, design: .monospaced)).foregroundColor(.textPrimary)
                Spacer()
            }
            Text("Controls the informational daily macro watchlist. It does not become a hard trade gate.")
            ...
            HStack(spacing: 8) {
                Button("SAVE NEWS SETTINGS") { controller.save() }
                    .font(.system(size: 9, weight: .bold, design: .monospaced)).foregroundColor(.bgPrimary)
                    .padding(.horizontal, 10).padding(.vertical, 6).background(Color.accentGold).cornerRadius(6).buttonStyle(.plain)
                Button("RELOAD") { controller.reload() }
                    .font(.system(size: 9, weight: .bold, design: .monospaced)).foregroundColor(.accentGold)
                    .padding(.horizontal, 10).padding(.vertical, 6).background(Color.accentGold.opacity(0.10)).cornerRadius(6).buttonStyle(.plain)
            }
        }
        .padding(.top, 4)
    }
    ...
    @ViewBuilder private func newsSlider(_ label: String, value: Binding<Double>, range: ClosedRange<Double>, step: Double) -> some View {
        HStack(spacing: 8) {
            Text(label).font(.system(size: 9)).foregroundColor(.textSecondary).lineLimit(1).minimumScaleFactor(0.75)
            Slider(value: value, in: range, step: step)
            Text(String(format: "%.1f", value.wrappedValue)).font(.system(size: 9, weight: .bold, design: .monospaced)).foregroundColor(.accentGold).frame(width: 45, alignment: .trailing)
        }
    }
}
