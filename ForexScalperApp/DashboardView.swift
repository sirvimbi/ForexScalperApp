...
struct DashboardView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .dismissSignalOverlay)) { _ in
            // Executed/denied signals are no longer actionable. Always dismiss the
            // presentation state after the coordinator updates its signal lifecycle.
            selectedSignal = nil
        ...
        case 5:
            SettingsView(viewModel: viewModel, coordinator: coordinator)
                ...
                .overlay(alignment: .topTrailing) {
                    MT5DisconnectControl(viewModel: viewModel, coordinator: coordinator).padding(.top, 16).padding(.trailing, 24)
                }
                #endif
...
struct PerformanceView: View {
    @ObservedObject var viewModel: DashboardViewModel
    private var completedTrades: [TradeRecord] { viewModel.tradeHistory.filter { $0.status == .completed }.sorted { ($0.exitTime ?? $0.entryTime) < ($1.exitTime ?? $1.entryTime) } }
    ...
    private var averageDurationMinutes: Double {
        let durations = completedTrades.compactMap { trade -> Double? in guard let exit = trade.exitTime else { return nil }; return exit.timeIntervalSince(trade.entryTime) / 60 }
    ...
    private var currentStreak: String {
        ...
        var count = 0
        for trade in completedTrades.reversed() { if ((trade.pnl ?? 0) > 0) == isWin { count += 1 } else { break } }
        return "\(isWin ? "W" : "L")\(count)"
    }
    private var maxDrawdownFromHistory: Double {
        var equity = 0.0, peak = 0.0, drawdown = 0.0
        for trade in completedTrades { equity += trade.pnl ?? 0; peak = max(peak, equity); drawdown = min(drawdown, equity - peak) }
        return abs(drawdown)
    }
    private var recoveryFactor: Double { let dd = maxDrawdownFromHistory; return dd > 0 ? viewModel.totalPnL / dd : viewModel.totalPnL }
            ...
            VStack(spacing: 18) {
                HStack {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text("Live broker history • execution quality • risk-adjusted diagnostics").font(.system(size: 10, design: .monospaced)).foregroundColor(.textMuted)
                    }
                    Spacer()
                    Text("\(completedTrades.count) CLOSED").font(.system(size: 9, weight: .bold, design: .monospaced)).foregroundColor(.textSecondary).padding(.horizontal, 9).padding(.vertical, 5).background(Color.white.opacity(0.05)).cornerRadius(5)
                }
                ...
                HStack(spacing: 16) {
                    performanceCard(title: "TRADE QUALITY", icon: "checkmark.seal.fill", color: .accentGreen) {
                        perfRow("Wins", "\(winningTrades.count)", .accentGreen); perfRow("Losses", "\(losingTrades.count)", .accentRed); perfRow("Avg Win", String(format: "%.2f", viewModel.avgWin), .accentGreen); perfRow("Avg Loss", String(format: "%.2f", viewModel.avgLoss), .accentRed); perfRow("Largest Win", String(format: "%.2f", largestWin), .accentGreen); perfRow("Largest Loss", String(format: "%.2f", largestLoss), .accentRed); perfRow("Current Streak", currentStreak, .accentGold)
                    }
                    performanceCard(title: "DIRECTIONAL EDGE", icon: "arrow.left.arrow.right", color: .accentPurple) {
                        directionRow("LONG", viewModel.totalBuyTrades, viewModel.totalBuyWins, viewModel.totalBuyPnL, .accentGreen); directionRow("SHORT", viewModel.totalSellTrades, viewModel.totalSellWins, viewModel.totalSellPnL, .accentRed)
                    }
                }
                performanceCard(title: "TRADING DIAGNOSTICS", icon: "waveform.path.ecg", color: .accentCyan) {
                    HStack(spacing: 24) { diagnostic("Gross Profit", String(format: "%.2f", grossProfit), .accentGreen); diagnostic("Gross Loss", String(format: "%.2f", grossLoss), .accentRed); diagnostic("Total Trades", "\(completedTrades.count)", .accentCyan); diagnostic("Daily P&L", String(format: "%@%.2f", viewModel.totalDailyPnL >= 0 ? "+" : "", viewModel.totalDailyPnL), viewModel.totalDailyPnL >= 0 ? .accentGreen : .accentRed) }
                }
            }.padding(24)
        }.background(Color.bgPrimary).onAppear { viewModel.loadTradeHistory() }
    }

    private func metricCard(_ title: String, _ value: String, _ color: Color, _ icon: String) -> some View {
        GlassCard(borderColor: color.opacity(0.25)) { HStack(spacing: 10) { Image(systemName: icon).font(.system(size: 14, weight: .bold)).foregroundColor(color); VStack(alignment: .leading, spacing: 4) { Text(title).font(.system(size: 8, weight: .bold, design: .monospaced)).foregroundColor(.textMuted); Text(value).font(.system(size: 15, weight: .black, design: .monospaced)).foregroundColor(color).lineLimit(1).minimumScaleFactor(0.7) }; Spacer() }.padding(12) }
    }
    private func performanceCard<Content: View>(title: String, icon: String, color: Color, @ViewBuilder content: () -> Content) -> some View {
        GlassCard(borderColor: color.opacity(0.2)) { VStack(alignment: .leading, spacing: 12) { HStack(spacing: 7) { Image(systemName: icon).foregroundColor(color); Text(title).font(.system(size: 10, weight: .bold, design: .monospaced)).foregroundColor(.textPrimary).tracking(1); Spacer() }; Divider().background(Color.borderSubtle); content() }.padding(16) }.frame(maxWidth: .infinity, alignment: .topLeading)
    }
    private func perfRow(_ label: String, _ value: String, _ color: Color) -> some View { HStack { Text(label).font(.system(size: 10)).foregroundColor(.textSecondary); Spacer(); Text(value).font(.system(size: 11, weight: .bold, design: .monospaced)).foregroundColor(color) } }
    private func directionRow(_ label: String, _ trades: Int, _ wins: Int, _ pnl: Double, _ color: Color) -> some View { VStack(alignment: .leading, spacing: 5) { HStack { Text(label).font(.system(size: 10, weight: .black, design: .monospaced)).foregroundColor(color); Spacer(); Text(String(format: "%@%.2f", pnl >= 0 ? "+" : "", pnl)).font(.system(size: 11, weight: .bold, design: .monospaced)).foregroundColor(pnl >= 0 ? .accentGreen : .accentRed) }; perfRow("Trades", "\(trades)", .textPrimary); perfRow("Win Rate", String(format: "%.1f%%", trades > 0 ? Double(wins) / Double(trades) * 100 : 0), .accentGreen) } }
    private func diagnostic(_ title: String, _ value: String, _ color: Color) -> some View { VStack(alignment: .leading, spacing: 4) { Text(title).font(.system(size: 8, weight: .bold, design: .monospaced)).foregroundColor(.textMuted); Text(value).font(.system(size: 12, weight: .bold, design: .monospaced)).foregroundColor(color) } }
}
...
struct NoSignalsView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            Spacer(); ZStack { Circle().stroke(Color.accentCyan.opacity(0.1), lineWidth: 2).frame(width: 80, height: 80); Image(systemName: "antenna.radiowaves.left.and.right").font(.system(size: 30)).foregroundColor(.accentCyan.opacity(0.3)) }
            VStack(spacing: 8) { Text("SCANNING FOR INSTITUTIONAL FLOW").font(.system(size: 14, weight: .bold, design: .monospaced)).foregroundColor(.textPrimary); Text(connectionStatus == "Connected" ? "Active WebSocket stream on Port 8890. Waiting for high-probability confluence." : "Waiting for MT5 bridge connection...").font(.system(size: 11)).foregroundColor(.textSecondary).multilineTextAlignment(.center).frame(maxWidth: 300) }
