...
enum SignalAccuracyEngine {
    ...
    static func assess(symbol: String, direction: SignalType, candles: [Kline]) -> Assessment {
        ...
        if chop >= settings.choppinessWarningThreshold {
            ...
            regime = "mean-reverting"; adjustment += settings.meanReversionRegimeAdjustment; reasons.append("mean-reversion regime")
        } else { regime = "transitional" }

        switch divergence {
        case .supporting: adjustment += settings.supportingDivergenceAdjustment; reasons.append("directional RSI divergence")
    ...
    private static func microReversalConfirmation(_ candles: [Kline], direction: SignalType) -> (confirmed: Bool, reason: String) {
        let recent = Array(candles.suffix(8)); guard recent.count >= 5 else { return (false, "reversal confirmation unavailable") }
