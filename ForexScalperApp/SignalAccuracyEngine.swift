...
enum SignalAccuracyEngine {
    ...
    static func assess(symbol: String, direction: SignalType, candles: [Kline]) -> Assessment {
        let settings = SignalAccuracyRuntimeCache.shared.snapshot()
        guard candles.count >= settings.minimumHistoryCandles, direction != .none else { return Assessment(approved: true, confidenceAdjustment: 0, regime: "unknown", choppiness: 50, hurst: 0.5, reasons: ["insufficient accuracy-layer history"]) }
        ...
        var adjustment = 0.0, reasons: [String] = [], approved = true
        let regime: String
        ...
        let posterior = SignalAccuracyBayesianRuntime.shared.snapshot(key: "\(symbol.uppercased()):\(direction)", priorWins: settings.bayesianPriorWins, priorLosses: settings.bayesianPriorLosses)
        adjustment += (posterior - 0.5) * settings.bayesianAdjustmentScale; reasons.append("Bayesian prior=\(Int(posterior * 100))%")
    ...
    static func recordOutcome(outcomeID: String, symbol: String, direction: SignalType, profitable: Bool) async {
        ...
        if didRecord { SignalAccuracyBayesianRuntime.shared.update(key: key, profitable: profitable, priorWins: settings.bayesianPriorWins, priorLosses: settings.bayesianPriorLosses) }
    ...
    private static func divergenceState(_ candles: [Kline], direction: SignalType, settings: SignalAccuracyConfiguration) -> Divergence {
        ...
        let halfCandles = max(1, recent.count / 2), first = Array(recent.prefix(halfCandles)), second = Array(recent.suffix(halfCandles)); let firstHigh = first.map(\.close).max() ?? 0, secondHigh = second.map(\.close).max() ?? 0, firstLow = first.map(\.close).min() ?? 0, secondLow = second.map(\.close).min() ?? 0
        ...
        let bearish = secondHigh > firstHigh && secondRSIHigh < firstRSIHigh - settings.divergenceRSIMargin, bullish = secondLow < firstLow && secondRSILow > firstRSILow + settings.divergenceRSIMargin
    ...
    private static func microReversalConfirmation(_ candles: [Kline], direction: SignalType) -> (confirmed: Bool, reason: String) {
        ...
        if direction == .buy { let dipped = (previous.map(\.close).max() ?? latest.close) > (previous.map(\.close).min() ?? latest.close); let bullish = latest.close > latest.open && latest.close > turn.high; return (dipped && bullish, dipped && bullish ? "dip → bullish reclaim confirmed" : "buy waiting for bullish reclaim") }
        if direction == .sell { let rallied = (previous.map(\.close).max() ?? latest.close) > (previous.map(\.close).min() ?? latest.close); let bearish = latest.close < latest.open && latest.close < turn.low; return (rallied && bearish, rallied && bearish ? "rally → bearish rejection confirmed" : "sell waiting for bearish rejection") }
    ...
    private static func relativeStrengthIndex(_ candles: [Kline], period: Int) -> [Double] {
        guard candles.count > period else { return [] }; var gains = 0.0, losses = 0.0; for i in 1...period { let d = candles[i].close - candles[i - 1].close; if d >= 0 { gains += d } else { losses -= d } }
        var result: [Double] = [], avgGain = gains / Double(period), avgLoss = losses / Double(period); result.append(avgLoss == 0 ? 100 : 100 - 100 / (1 + avgGain / avgLoss))
        if candles.count > period + 1 { for i in (period + 1)..<candles.count { let d = candles[i].close - candles[i - 1].close; avgGain = (avgGain * Double(period - 1) + max(d, 0)) / Double(period); avgLoss = (avgLoss * Double(period - 1) + max(-d, 0)) / Double(period); result.append(avgLoss == 0 ? 100 : 100 - 100 / (1 + avgGain / avgLoss)) } }; return result
    }

    private static func choppinessIndex(_ candles: [Kline], period: Int) -> Double {
        guard candles.count > period else { return 50 }; let recent = Array(candles.suffix(period + 1)); var tr = 0.0; for i in 1..<recent.count { tr += max(recent[i].high - recent[i].low, abs(recent[i].high - recent[i - 1].close), abs(recent[i].low - recent[i - 1].close)) }; let range = (recent.map(\.high).max() ?? 0) - (recent.map(\.low).min() ?? 0); guard tr > 0, range > 0 else { return 100 }; return 100 * log10(tr / range) / log10(Double(period))
    }

    private static func hurstExponent(_ prices: [Double]) -> Double {
        guard prices.count >= 40 else { return 0.5 }; let sample = Array(prices.suffix(100)); var xs: [Double] = [], ys: [Double] = []; let maxLag = min(sample.count / 4, 20); guard maxLag >= 5 else { return 0.5 }
        for lag in 5...maxLag { var rs: [Double] = [], start = 0; while start + lag <= sample.count { let segment = Array(sample[start..<(start + lag)]), mean = segment.reduce(0, +) / Double(segment.count); var cumulative = 0.0, minC = 0.0, maxC = 0.0; for value in segment { cumulative += value - mean; minC = min(minC, cumulative); maxC = max(maxC, cumulative) }; let sd = sqrt(segment.map { ($0 - mean) * ($0 - mean) }.reduce(0, +) / Double(segment.count)); if sd > 0 { rs.append((maxC - minC) / sd) }; start += lag }; if !rs.isEmpty { xs.append(log(Double(lag))); ys.append(log(rs.reduce(0, +) / Double(rs.count))) } }
        guard xs.count >= 3 else { return 0.5 }; let mx = xs.reduce(0, +) / Double(xs.count), my = ys.reduce(0, +) / Double(ys.count), num = zip(xs, ys).map { ($0 - mx) * ($1 - my) }.reduce(0, +), den = xs.map { ($0 - mx) * ($0 - mx) }.reduce(0, +); guard den > 0 else { return 0.5 }; return max(0, min(1, num / den))
    }

    private static func sessionMultiplier(for date: Date, settings: SignalAccuracyConfiguration) -> Double {
        let hour = Calendar(identifier: .gregorian).component(.hour, from: date); let favorable = (hour >= settings.favorableSession1StartHour && hour <= settings.favorableSession1EndHour) || (hour >= settings.favorableSession2StartHour && hour <= settings.favorableSession2EndHour); let reduced = settings.reducedSessionStartHour <= settings.reducedSessionEndHour ? hour >= settings.reducedSessionStartHour && hour <= settings.reducedSessionEndHour : hour >= settings.reducedSessionStartHour || hour <= settings.reducedSessionEndHour; if favorable { return settings.favorableSessionMultiplier }; if reduced { return settings.reducedSessionMultiplier }; return 1
    }
}

final class SignalAccuracyBayesianRuntime: @unchecked Sendable {
    static let shared = SignalAccuracyBayesianRuntime()
    private struct Bucket: Codable { var wins: Double; var losses: Double }; private struct Persisted: Codable { var buckets: [String: Bucket] }
...
private actor SignalAccuracyBayesianStore {
    ...
    private struct Bucket: Codable { var wins: Double; var losses: Double }; private struct Persisted: Codable { var buckets: [String: Bucket]; var recordedOutcomeIDs: Set<String> }
